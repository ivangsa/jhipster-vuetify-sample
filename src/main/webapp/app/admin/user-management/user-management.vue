...
<div>
  ...
  <v-data-table :items="orderBy(users, propOrder, reverse === true ? 1 : -1)" :total-items="totalItems" :pagination.sync="pagination" class="elevation-1">
    ...
    <template slot="items" slot-scope="props">
        <td><router-link tag="a" :to="{name: 'JhiUserView', params: {userId: props.item.login}}">{{props.item.id}}</router-link></td>
        <td>{{props.item.login}}</td>
        <td>{{props.item.email}}</td>
        <td>
            <button class="btn btn-danger btn-sm" v-on:click="setActive(user, true)" v-if="!props.item.activated"
                    v-text="$t('userManagement.deactivated')">Deactivated</button>
            <button class="btn btn-success btn-sm" v-on:click="setActive(user, false)" v-if="props.item.activated"
                    :disabled="username === props.item.login" v-text="$t('userManagement.activated')">Activated</button>
        </td>
        <td>{{props.item.langKey}}</td>
        <td>
            <div v-for="authority of props.item.authorities" :key="authority">
                <span class="badge badge-info">{{ authority }}</span>
            </div>
        </td>
        <td>{{props.item.createdDate | formatDate}}</td>
        <td>{{props.item.lastModifiedBy}}</td>
        <td>{{props.item.lastModifiedDate | formatDate}}</td>
        <td class="text-right">
          <v-icon small color="indigo" @click="$router.push({name: 'JhiUserView', params: {userId: props.item.login}})">visibility</v-icon>        
          <v-icon small color="green" @click="$router.push({name: 'JhiUserEdit', params: {userId: props.item.login}})">edit</v-icon>     
          <v-icon small color="red" @click="prepareRemove(props.item)" :disabled="username === props.item.login">delete</v-icon>
        </td>
    </template>
  </v-data-table>
